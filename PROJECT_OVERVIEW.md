...
- ✅ سیستم تخصیص حساب‌های مقصد (DepositAccountAssignment)
  ...
  - **پرداخت جزئی (Partial Payment):** امکان تخصیص جزئی به درخواست‌های برداشت ناقص
...
- ✅ تایید خودکار درخواست‌های برداشت (وقتی واریز انجام می‌شود)
  - **پرداخت کامل:** تایید خودکار وقتی کل مبلغ پرداخت می‌شود
  - **پرداخت جزئی:** نمایش باقی‌مانده و مبلغ پرداخت شده
- ✅ **تکمیل برداشت‌های ناقص:**
  - امکان تکمیل توسط واریز کاربران دیگر
  - نمایش باقی‌مانده در لیست تخصیص
  - تخصیص فقط باقی‌مانده (نه مبلغ کل)
  - تایید خودکار پس از تکمیل کامل
- ✅ نمایش فیش‌های واریزی در پنل مدیریت و کاربری
  - **پنل کاربری:** نمایش همه فیش‌های واریزی در تاریخچه
  - **پنل مدیریت:** نمایش فیش‌های پرداخت‌شده برای برداشت‌های ناقص
  - نمایش وضعیت هر فیش (تایید شده/در انتظار/رد شده)
  - نمایش اطلاعات حساب مقصد برای هر فیش
- ✅ تسویه درخواست‌های برداشت طلا
- ✅ تاریخچه تراکنش‌ها (واریز و برداشت)
  - نمایش باقی‌مانده و مبلغ پرداخت شده برای برداشت‌های ناقص
  - Progress bar برای نمایش درصد تکمیل
  - نمایش همه فیش‌های مرتبط (در صورت تکمیل توسط چند واریز)
...
4. **تایید خودکار درخواست‌های برداشت:**
   ...
   - درخواست برداشت به صورت خودکار تایید می‌شود (فقط اگر کل مبلغ پرداخت شده باشد)
   - موجودی مسدود شده آزاد می‌شود
   - پیامک به کاربر درخواست کننده برداشت ارسال می‌شود

5. **پرداخت جزئی (Partial Payment):**
   - امکان تخصیص جزئی به درخواست‌های برداشت
   - اگر مبلغ واریز کمتر از مبلغ برداشت باشد، فقط بخشی تخصیص داده می‌شود
   - برداشت ناقص در حالت PENDING می‌ماند
   - نمایش باقی‌مانده و مبلغ پرداخت شده در پنل مدیریت و کاربری
   - Progress bar برای نمایش درصد تکمیل

6. **تکمیل برداشت‌های ناقص:**
   - امکان تکمیل برداشت ناقص توسط واریز کاربران دیگر
   - نمایش باقی‌مانده در لیست تخصیص (نه مبلغ کل)
   - تخصیص فقط باقی‌مانده (سیستم به صورت خودکار باقی‌مانده را محاسبه می‌کند)
   - تایید خودکار پس از تکمیل کامل (وقتی باقی‌مانده = 0)
   - نمایش همه فیش‌های مرتبط (از واریزکنندگان مختلف) در پنل کاربری

7. **نمایش فیش‌های واریزی:**
   - **پنل مدیریت:**
     - در مودال جزئیات درخواست برداشت (برای درخواست‌های خودکار تایید شده)
     - نمایش فیش‌های پرداخت‌شده برای برداشت‌های ناقص
     - نمایش وضعیت پرداخت (مبلغ کل، پرداخت شده، باقی‌مانده)
   - **پنل کاربری:**
     - در تب تاریخچه تراکنش‌ها: نمایش همه فیش‌های واریزی برای هر درخواست واریز
     - نمایش همه فیش‌های مرتبط برای برداشت‌های تکمیل شده (از واریزکنندگان مختلف)
     - نمایش وضعیت هر فیش (تایید شده/در انتظار/رد شده) با badge رنگی
     - نمایش اطلاعات حساب مقصد برای هر فیش
     - نمایش باقی‌مانده و مبلغ پرداخت شده برای برداشت‌های ناقص
...
8. **بهبودهای UI/UX:**
   ...
   - نمایش وضعیت فیش‌های آپلود شده در تب واریز
   - Progress bar برای نمایش درصد تکمیل برداشت‌های ناقص

9. **سیستم پیامک:**
   - پیامک به کاربر بعد از تایید درخواست واریز (`deposit-approved-user`)
   - پیامک به مدیر بعد از آپلود فیش‌های واریزی (`deposit-receipt-uploaded-admin`)
   - پیامک به کاربر درخواست کننده برداشت بعد از تایید خودکار (`withdrawal-approved-user`)

10. **بهینه‌سازی دیتابیس:**
    - نگه داشتن `assignments` برای حفظ `receipts` و `links` (دیگر حذف نمی‌شوند)
    - استفاده از `prefetch_related` برای بهینه‌سازی query ها
    - استفاده از `transaction.atomic()` برای اطمینان از atomicity
    - محاسبه باقی‌مانده درخواست‌های برداشت با متد `get_remaining_amount()`
...
## 📋 تغییرات اخیر (1404/10/17)

### ✅ **بهبود سیستم واریز و برداشت - پرداخت جزئی و تکمیل برداشت‌های ناقص:**

1. **پرداخت جزئی (Partial Payment):**
   - امکان تخصیص جزئی به درخواست‌های برداشت
   - اگر مبلغ واریز کمتر از مبلغ برداشت باشد، فقط بخشی تخصیص داده می‌شود
   - برداشت ناقص در حالت PENDING می‌ماند تا کامل شود
   - نمایش باقی‌مانده و مبلغ پرداخت شده در پنل مدیریت و کاربری
   - Progress bar برای نمایش درصد تکمیل

2. **تکمیل برداشت‌های ناقص:**
   - امکان تکمیل برداشت ناقص توسط واریز کاربران دیگر
   - نمایش باقی‌مانده در لیست تخصیص (نه مبلغ کل)
   - تخصیص فقط باقی‌مانده (سیستم به صورت خودکار باقی‌مانده را محاسبه می‌کند)
   - تایید خودکار پس از تکمیل کامل (وقتی باقی‌مانده = 0)
...
3. **بهبود نمایش فیش‌های واریزی:**
   - **پنل کاربری - تب تاریخچه:**
     - نمایش همه فیش‌های واریزی برای هر درخواست واریز
     - نمایش اطلاعات حساب مقصد برای هر فیش
     - نمایش وضعیت هر فیش (تایید شده/در انتظار/رد شده) با badge رنگی
     - نمایش باقی‌مانده و مبلغ پرداخت شده برای برداشت‌های ناقص
     - Progress bar برای نمایش درصد تکمیل
   - **پنل کاربری - تب واریز:**
     - نمایش فیش‌های آپلود شده با وضعیت آن‌ها
     - نمایش اطلاعات حساب مقصد برای هر فیش
   - **پنل مدیریت:**
     - نمایش وضعیت پرداخت در مودال برداشت (مبلغ کل، پرداخت شده، باقی‌مانده)
     - Progress bar برای نمایش درصد تکمیل
     - نمایش فیش‌های پرداخت‌شده برای برداشت‌های ناقص

4. **سناریوهای پشتیبانی شده:**
   - **سناریو 1:** واریز بیشتر از برداشت‌ها
     - تخصیص کامل به چند برداشت + حساب پیش‌فرض
     - تایید خودکار همه برداشت‌ها
   - **سناریو 2:** واریز کمتر از برداشت‌ها
     - تخصیص جزئی به برداشت‌ها
     - تکمیل برداشت‌های ناقص توسط واریزهای بعدی
     - نمایش همه فیش‌های مرتبط پس از تکمیل

5. **بهبودهای Backend:**
   - اصلاح `admin_approve_deposit_new_flow` برای پردازش پرداخت‌های جزئی
   - اصلاح `admin_approve_withdrawal` برای تکمیل برداشت‌های ناقص
   - استفاده از متد `get_remaining_amount()` برای محاسبه باقی‌مانده
   - استفاده از پیامک‌های موجود (بدون ایجاد template جدید)

6. **بهبودهای Frontend:**
   - اصلاح منطق تخصیص برای در نظر گرفتن باقی‌مانده برداشت‌های ناقص
   - نمایش باقی‌مانده در لیست تخصیص
   - بهبود UI برای نمایش وضعیت فیش‌ها و پرداخت‌های جزئی

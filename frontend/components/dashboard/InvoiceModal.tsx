    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-[620px] rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl">
        ...
        <div className="overflow-y-auto bg-gray-100 p-4 flex justify-center">
          {/* قالب A5 هم‌راستا با PDF */}
          <div
            className="bg-white w-[148mm] min-h-[210mm] border-2 border-black p-3 text-black relative flex flex-col shadow-lg print:shadow-none"
              ...
              <div className="flex justify-between items-start border-b-2 border-black pb-3 mb-4">
                <div className="flex items-center gap-3">
                  {issuer?.logo_url ? (
                    ...
                    <img src={issuer.logo_url} alt={brandName} className="h-10 w-auto object-contain" />
                  ) : (
                    <BrandLogo variant="mark" size={40} showName={false} />
                  )}
                  <div>
                    <h1 className="font-black text-base text-gray-900">{brandName}</h1>
                    <p className="text-[10px] text-gray-600 mt-0.5">{tagline}</p>
                  </div>
                </div>
              ...
              <div className="flex justify-end mb-6">
                <div className="w-[65%] border-2 border-black text-xs">
                  <div className="flex border-b border-black">
                    <span className="w-[35%] bg-gray-100 font-bold p-2 border-l border-black">جمع کل:</span>
                    <span className="flex-1 font-black p-2 text-left">
                      {toPersianDigits(Number(data.total).toLocaleString())}
                    </span>
                  </div>
                  <div className="flex border-b border-black">
                    <span className="w-[35%] bg-gray-100 font-bold p-2 border-l border-black">مالیات:</span>
                    <span className="flex-1 font-black p-2 text-left">۰</span>
                  </div>
                  <div className="flex bg-gray-100">
                    <span className="w-[35%] bg-gray-200 font-bold p-2 border-l border-black">قابل پرداخت:</span>
                    <span className="flex-1 font-black p-2 text-left">
                      {toPersianDigits(Number(data.total).toLocaleString())}{" "}

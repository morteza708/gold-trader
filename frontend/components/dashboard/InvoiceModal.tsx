        ...
        <div className="overflow-y-auto bg-gray-100 p-4 flex justify-center">
          ...
          >
            <div className="flex justify-between items-start border-b-2 border-gray-800 pb-4 mb-6">
              <div className="flex items-center gap-3">
                {issuer?.logo_url ? (
                  ...
                  <img src={issuer.logo_url} alt={brandName} className="h-12 w-auto object-contain" />
                ) : (
                  <BrandLogo variant="mark" size={48} showName={false} />
                )}
                <div>
                  <h1 className="font-black text-xl text-gray-900">{brandName}</h1>
              ...
              <div className="text-left text-sm space-y-1">
                ...
                {data.channel === "MANUAL" && (
                  <p className="text-xs text-amber-700 font-bold">فاکتور دستی / حضوری</p>
                )}
            ...
            <table className="w-full mb-8">
              ...
              <tbody>
                <tr>
                  <td className="py-4 border-b border-gray-100 text-sm px-2 font-bold">
                    {isBuy ? "خرید" : "فروش"} طلای آب‌شده
                  </td>
                  <td className="py-4 border-b border-gray-100 text-center text-sm px-2">۷۵۰ (۱۸)</td>
                  <td className="py-4 border-b border-gray-100 text-center text-sm px-2 font-bold">
                    {toPersianDigits(Number(data.amount).toFixed(3))}
                  </td>
                  <td className="py-4 border-b border-gray-100 text-center text-sm px-2">
                    {toPersianDigits(Number(data.price).toLocaleString())}
                  </td>
                  <td className="py-4 border-b border-gray-100 text-left text-sm px-2 font-black">
